...
def create_input_files(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
    ...
    with open("./data/test_image_paths.json", 'wb') as json_file:  # FIXME:  Store the test img paths
    ...
    with open(os.path.join(output_folder, 'WORDMAP_' + base_filename + '.json'), 'wb') as j:
    ...
    for impaths, imcaps, split in [(train_image_paths, train_image_captions, 'TRAIN'),
        ...
        with h5py.File(os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'), 'a') as h:
            ...
            with open(os.path.join(output_folder, split + '_CAPTIONS_' + base_filename + '.json'), 'wb') as j:
                json.dump(enc_captions, j)

            with open(os.path.join(output_folder, split + '_CAPLENS_' + base_filename + '.json'), 'wb') as j:
                json.dump(caplens, j)
...
def save_checkpoint(checkpoint_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    filepath = os.path.join('./last_models_weights/', filename)  # 最终参数模型
    torch.save(state, filepath)

    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        torch.save(state, os.path.join('./best_models_weights/', 'BEST_' + filename))
